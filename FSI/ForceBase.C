...
: mesh_(mesh),
  ...
  preciceDict_(mesh.lookupObject<Foam::IOdictionary>("preciceDict")),
  atmP_(0.0)
{
    ...
    // for atmosphere pressure
    if (preciceDict_.found("FSI"))
    {
        const Foam::dictionary& fsiDict = preciceDict_.subDict("FSI");
        if (fsiDict.found("atmPressure"))
            atmP_ = Foam::readScalar(fsiDict.lookup("atmPressure"));
    }
}
                                                          ...
                                                          const unsigned int dim) const
{
    ...
    // scalar atmP = 0.0; // default 0, [Pa] read from preciceDict FSI subDict if available
    ...
    for (const label patchID : patchIDs_)
    {
        ...
        {           
            // // calculate the pressure forces after substracting the atmosphere pressure
            forceField.boundaryFieldRef()[patchID] = surface * (pb[patchID] - atmP_);
            
            // the origional metod to calculate pressure forces
            // forceField.boundaryFieldRef()[patchID] = surface * pb[patchID];
        }
        else
        {
            FatalErrorInFunction
